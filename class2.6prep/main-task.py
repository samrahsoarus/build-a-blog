...
# Trace tasks with global variable that can be accessed by any request handler in file
tasks = []

# Check if handling 'GET' (rendering) or 'POST' (processing) request
@app.route('/todos', methods=['POST', 'GET'])
def todos():

    if request.method == 'POST':
        # Get given task out of request and add it
        task = request.form['task']
        tasks.append(task)

    template = jinja_env.get_template('todos.html')
    # Pass data that can be inserted into the base template
